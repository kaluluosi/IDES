...
!!! info "这一章讲什么"
    内置工具是**自带能力**（怎么用），见 [内置工具](../builtin-tools/)。强袭挂件是**扩展能力**（怎么加装备）——让 IDES 不再局限于出厂自带的那些工具。
...
> 关于 mise 本身（不随 IDES 装、agent 自己装、配合挂件），见 [内置工具](../builtin-tools/)。
...
!!! tip "延伸阅读"
    想扩展更强的文件能力（搜索、处理），就是走这条 custom tool 的路子注册 `rg` / `es.exe` / `ast-grep-mcp` / `uffs` 进来。关于内置的文件工具，见 [内置工具](../builtin-tools/)。
