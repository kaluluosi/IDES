...
IDES 是 **agent 自治**的设计理念——**IDES 的 agent 可以自己管理和配置 IDES**。

所以 IDES 的正确使用方式，**不是用户去配置一切**，而是：

> 跟 agent 聊，告诉 agent 你想做什么，然后由 **agent 自己去配置自己**。

这也是为什么 IDES 的**配置项很少**的原因。IDES 就像**操作系统**一样——一切都可以在跟 agent 交流的过程中，让 agent 代劳。

!!! tip "这一章讲什么"
    既然 agent 能自己配置自己，那设置界面里**每一项是什么、什么时候才需要动**，就值得先弄明白。下面从模型、代理、记忆到感知，逐一拆开讲。
